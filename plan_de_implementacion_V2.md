# Prompt: 
Perfecto, vamos a trabajar con la app. Me gustaria desarrollar una app movil con Flutter/Dart llamada "LifeTours", consiste en una aplicacion para agendar tours con todas las funciones que vienen implicitas en las tablas, excepto que no son necesarias las notificaciones. La base de datos se hara en firebase Firestore, no se habilitara Google Analythics y se usara la version estandar, (Dime si esta bien o conviene usar otro gestor de base de datos). La app realiza acciones completamente digitales, el acceder a un guia en persona o asistir a un tour son acciones propias del usuario que no se toman en cuenta para el desarrollo, el punto es que el usuario pueda agendar e interactuar con los tours de forma virtual para el luego reclamar sus servicios de forma presencial ajenamente a la app. Igualmente la app incluira la funcion de acceder como administrador, con esto se podra acceder a las tablas del proyecto y realizar las operaciones CRUD con cada una de ellas. DE MOMENTO SOLO ES NECESARIO REALIZAR UN PLAN DE IMPLEMENTACION, te adjunto un plan anterior para realizarle mejoras y asegurar que el desarrollo del proyecto sea el correcto. Entre las herramientas que se requieren, UI, UX, dependencias, login autenticacion usuario password, base de datos firestore, privider, dependencias en pubsec.yaml. El proyecto no es profesional, solo tiene fines academicos pero debe ser completamente funcional. Recuerda, de momento solo hay que hacer el plan de implementacion, en el me gustaria que añadieras la estructura del proyecto.
